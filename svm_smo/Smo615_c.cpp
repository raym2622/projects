...
// 12/13/18
//
// The four input arguments (argv) are the training data, training label, test data, and test label files
// The three input parameters (cin) are the regularization parameter C, the kernel selection ind (0 or 1), and pred_labelfor printing the predicted labels (1 -> yes)
...
int main(int argc, char* argv[]) {
    ...
    double c = numbers[0]; // user provides the regularization parameter
    int ind = (int)numbers[1]; // user provides the kernel (0 for linear, 1 for Gaussian)
    int pred_label = (int)numbers[2]; // user can choose to print the predicted labels (1 yes)
    
    MatrixXd init_X = readFromFile<double>(argv[1]);
    MatrixXd init_y = readFromFile<double>(argv[2]);
    MatrixXd X_test = readFromFile<double>(argv[3]);
    VectorXd y_test = readFromFile<double>(argv[4]);
