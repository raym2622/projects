...
def get_fold_indices(n, num_folds, fold_id):
    ...
    If fold_id is num_folds-1 (last fold)
    ...
    if fold_id == num_folds-1:
        train_indices = range(0, fold_id * fold_size)
    else:
        train_indices = range(0, fold_id * fold_size) + \
            range((fold_id + 1) * fold_size, n)

    # set test_indices to those indices in range(n)
    # that are not in train_indices
    test_indices = [x for x in range(0,n) if x not in train_indices]
...
def nn_classifier(point, train_data, train_labels, k, K):
    ...
    nearest_k_labels = [x[j][2] for x in all_three_sorted for j in range(0,k)]
...
def classification_error(classifier, data, labels):
    ...
    total_error = error_indicators.count(True)
...
def main():
    ...
    for k in k_range:
        ...
        for fold_id in range(num_folds):
            ...
            train_labels = [labels[j] for j in fold_train_indices]
            classifier = lambda x: nn_classifier(x, train_data, \
                    train_labels, k, K)

            # get the test data and labels
            # and evaluate the classifier's error
            test_data = [instances[i] for i in fold_test_indices]
            test_label = [labels[j] for j in fold_test_indices]
